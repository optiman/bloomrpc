...
export async function parse(serviceError: ServiceError): Promise<ResponseError> {
    ...
    const defaultErrorProtoPath = path.join(
        (() => remote.app.isPackaged ? remote.process.resourcesPath : process.cwd())(),
        "static/google/rpc");
    const root = await getRoot([...getImportPaths(), defaultErrorProtoPath]);
